...
import odmservices.utilities as util
...
class ServiceManager():
    ...
    def __build_connection_string(self, conn_dict):
        ...
        if conn_dict['engine'] == 'mssql' and sys.platform != 'win32':
            driver = "pyodbc"
            quoted = urllib.parse.quote_plus('DRIVER={ODBC Driver 18 for SQL Server};SERVER=%s;DATABASE=%s;UID=%s;PWD=%s;TrustServerCertificate=yes;' % (conn_dict['address'], conn_dict['database'], conn_dict['user'], conn_dict['password']))
